...
## If the variable interface exists, add it to an array, else read all interfaces from the system dynamically
[ -v interface ] && interfaces=( $interface ) || interfaces=( $(netstat -i | tail -n+3 | awk '{print $1}') )
