...
This program generates a report on key system. The options below can be used to limit the report generated.
 -h display this help manual
 --cpu only show information about the CPU
 --memory only show information about the memory (RAM)
 --host only show information on the host
 --domain only show information on the domain
 --disk only show information on the disk space
 --printer only show information on the printers attached to the system
 --ipconfig only show information on the IP addresses linked to the system
 --os only show information related to the operating system
"
}

function cleanup (){
	# Using wildcards to find all files that match the pattern e.g /tmp/somethinginfo.$$
	rm -f /tmp/*info.*
}
